...
export default class WavesApi extends KuroClient {
  ...
  async loginStatusCheck() {
    try {
      ...
      logger.warn(`[ams] login_status_check: ${JSON.stringify(res)}`)
      return { status: false, code: res.code, msg: res.msg }
    } catch (error) {
      logger.error(`[ams] login_status_check: ${error}`)
      return { status: false, msg: "检查登录状态异常" }
    }
  }
  ...
  async refreshData() {
    try {
      ...
      logger.error(`[ams] refreshData: ${error}`)
      return { status: false, msg: "刷新数据异常" }
    }
  }
  ...
  async getRequestToken() {
    try {
      ...
      if (res.code === 200 && res.data?.accessToken) {
        return { status: true, bat: res.data.accessToken }
      }

      logger.warn(`[ams] getRequestToken: ${JSON.stringify(res)}`)
      return { status: false, bat: "", msg: res.msg }
    } catch (error) {
      logger.error(`[ams] getRequestToken: ${error}`)
      return { status: false, bat: "", msg: "获取bat异常" }
    }
  }
  ...
  async getRoleDetail(charId) {
    try {
      ...
      logger.error(`[ams] getRoleDetail: ${error.message}`)
      return { status: false, msg: "获取角色明细异常" }
    }
  }
  ...
  async getRoleData() {
    try {
      ...
      return { status: false, msg: res.msg || "获取角色列表失败" }
    } catch (error) {
      logger.error(`[ams] getRoleData: ${error.message}`)
      return { status: false, msg: "获取角色列表异常" }
    }
  }
  ...
  async getTowerDetail() {
    try {
      ...
      logger.error(`[ams] getTowerDetail: ${error}`)
      return { code: -1, msg: "获取深塔详情异常" }
    }
  }

  async getTowerIndex() {
    try {
      const indexRes = await this._post(
      ...
      logger.error(`[ams] getTowerIndex: ${error}`)
      return { code: -1, msg: "获取深塔首页异常" }
    }
  }
  ...
  async getSlashDetail() {
    try {
      ...
      logger.error(`[ams] getSlashDetail: ${error}`)
      return { code: -1, msg: "获取冥歌海墟详情异常" }
    }
  }
  ...
  async getSlashIndex() {
    try {
      ...
      logger.error(`[ams] getSlashIndex: ${error}`)
      return { code: -1, msg: "获取冥歌海墟首页异常" }
    }
  }
  ...
  async getChallengeDetail() {
    try {
      ...
      logger.error(`[ams] getChallengeDetail: ${error}`)
      return { code: -1, msg: "获取战术全息详情异常" }
    }
  }
  ...
  async getSlashData() {
    try {
      ...
      if (detailRes.code === 200 && !detailRes.data) {
        ...
        logger.warn(`[ams] getSlashData (index): ${JSON.stringify(indexRes)}`)
        return { status: false, msg: indexRes.msg || "获取冥歌海墟数据失败" }
      }

      logger.warn(`[ams] getSlashData: ${JSON.stringify(detailRes)}`)
      return { status: false, msg: detailRes.msg || "获取冥歌海墟数据失败" }
    } catch (error) {
      logger.error(`[ams] getSlashData: ${error}`)
      return { status: false, msg: "获取冥歌海墟数据异常" }
    }
  }
  ...
  async getTowerData() {
    try {
      ...
      if (detailRes.code === 200 && !detailRes.data) {
        ...
        logger.warn(`[ams] getTowerData (index): ${JSON.stringify(indexRes)}`)
        return { status: false, msg: indexRes.msg || "获取逆境深塔数据失败" }
      }

      logger.warn(`[ams] getTowerData: ${JSON.stringify(detailRes)}`)
      return { status: false, msg: detailRes.msg || "获取逆境深塔数据失败" }
    } catch (error) {
      logger.error(`[ams] getTowerData: ${error}`)
      return { status: false, msg: "获取逆境深塔数据异常" }
    }
  }
  ...
  async getSignInData() {
    try {
      ...
      if (listRes.code === 200) {
        return { status: true, data: listRes.data }
      }

      logger.warn(`[ams] getSignInData: ${JSON.stringify(listRes)}`)
      return { status: false, msg: listRes.msg || "获取签到信息失败" }
    } catch (error) {
      logger.error(`[ams] getSignInData: ${error}`)
      return { status: false, msg: "获取签到信息异常" }
    }
  }
  ...
  async signIn() {
    try {
      // 1. 执行签到
      const signRes = await this._post(
        WAVES_URLS.SIGN_IN,
        {
          gameId: this.gameId,
          serverId: this.serverId,
      ...
      if (signRes.code === 200) {
        ...
        logger.warn(`[ams] signIn: ${JSON.stringify(signRes)}`)
        return { status: false, msg: signRes.msg || "签到失败" }
      }
    } catch (error) {
      logger.error(`[ams] signIn: ${error}`)
      return { status: false, msg: "签到请求异常" }
    }
  }
  ...
  async getWidgetRefresh() {
    try {
      ...
      return { status: false, msg: res.msg || "获取组件数据失败" }
    } catch (error) {
      logger.error(`[ams] getWidgetRefresh: ${error}`)
      return { status: false, msg: "获取组件数据异常" }
    }
  }
  ...
  async getGachaLog(cardPoolType, recordId) {
    try {
      ...
      logger.error(`[ams] getGachaLog: ${e}`)
      return { code: -1, msg: "获取抽卡记录异常" }
    }
  }
  async getEventList(eventType = 0) {
    try {
      ...
      return { status: false, msg: res.msg || "获取公告列表失败" }
    } catch (error) {
      logger.error(`[ams] getEventList: ${error}`)
      return { status: false, msg: "获取活动列表异常" }
    }
  }
  ...
  async getPostDetail(postId) {
    try {
      ...
      return { status: false, msg: res.msg || "获取公告详情失败" }
    } catch (error) {
      logger.error(`[ams] getPostDetail: ${error}`)
      return { status: false, msg: "获取公告详情异常" }
    }
  }
}
