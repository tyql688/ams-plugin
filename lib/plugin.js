...
export class AmsPlugin extends plugin {
  ...
  async getImageUrl() {
    ...
    const fetch = async id => {
      ...
      try {
        ...
        for (const i of list) await crawl(i?.message)
      } catch (err) {
  ...
  render(renderPath, renderData = {}, cfg = {}) {
    ...
    const scaleCfg = cfg.scale ?? _.get(renderCfg, "scale", 100)
    const scale = Math.min(2, Math.max(0.5, scaleCfg / 100))
    const pct = `style='transform:scale(${scale})'`
    const layoutPathFull = path.join(resourcePath, "common", "layout")

    const { scale: _omitScale, ...runtimeCfg } = cfg
