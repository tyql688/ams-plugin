...
class LoginServer extends EventEmitter {
  ...
  async send404(res, title, message) {
    res.setHeader("Content-Type", "text/html; charset=utf-8")
    try {
      const notFoundHtmlPath = path.join(templatePath, "404.html")
      const html = (await fs.readFile(notFoundHtmlPath, "utf-8"))
        .replace(/{{ title }}/g, title)
        .replace(/{{ message }}/g, message)
      return res.status(404).send(html)
    } catch (e) {
      logger.error(`[ams] 404 Template Error: ${e}`)
      return res.status(404).send(`<h1>${title}</h1><p>${message}</p>`)
    }
  }
  ...
  _initApp() {
    ...
    this.app.get("/waves/i/:auth", async (req, res) => {
      ...
      if (!this.data.has(auth)) {
        return this.send404(res, "链接已失效", "请重新发起登录请求以获取新的链接")
      }
      ...
      if (Date.now() - entry.timestamp > this.getSessionTimeout()) {
        ...
        return this.send404(res, "链接已过期", "请重新发起登录请求以获取新的链接")
      }
    ...
    this.app.use((req, res) => {
      return this.send404(res, "页面不存在", "请检查链接是否正确，或重新发起登录请求")
    })
  }
